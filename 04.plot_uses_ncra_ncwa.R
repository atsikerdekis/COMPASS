...
system(paste0(path_NCO,"ncwa -O -a longitude,latitude ",expfile1_mergetime," ",expfile1_ncwa))
system(paste0(path_NCO,"ncra -O ", expfile1_mergetime," ",expfile1_ncra))
### remove temp  files
...
### Spatial (ncwa) and temporal (ncra) mean
system(paste0(path_NCO,"ncwa -O -a longitude,latitude ",expfile2_mergetime," ",expfile2_ncwa))
system(paste0(path_NCO,"ncra -O ", expfile2_mergetime," ",expfile2_ncra))
### remove temp  files
system(paste0("rm ",path_temp,"temp*"))
...
############
dpi <- 300
png(file_out, width=(0.2+3*3.9+2*0.7)*dpi, height=(0.23+0.15+2+2.5)*dpi)
...
myl <- layout(mat=matrix(c(1,1,1,1,1,1,2:13,14,14,14,14,14,14),4,6,byrow=T), widths=c(0.2,3.9,3.9,0.7,3.9,0.7), heights=c(0.23,0.15,2,2.5))
layout.show(myl)
...
par(mai=c(0,0,0,0));plot.new();text(0.5,0.5, paste0(expname1," (",exptype1,")"), col="grey20", cex=5, family="Century Gothic", srt=0)
par(mai=c(0,0,0,0));plot.new();text(0.5,0.5, paste0(expname2," (",exptype2,")"), col="grey20", cex=5, family="Century Gothic", srt=0)
par(mai=c(0,0,0,0));plot.new()
...
#for (v in 1:nvar) {
  ...
  if (variable_name[v] == "mec550") {
    ...
    rm(tmean_temp1, tmean_temp2)
    print(tmean_var1)
    print(max(tmean_var1))
    print(min(tmean_var1))
  }
  nc_close(file.nc)
  ### EXP2 ncra
  ...
  if (variable_name[v] == "mec550") {
    ...
    rm(tmean_temp1, tmean_temp2)
    print(tmean_var2)
    print(max(tmean_var2))
    print(min(tmean_var2))
  }
  ...
  mtext("Time (3 hourly)", side=1, line=12, cex=3.5)
  ...
  axis(side=1, at=x[IDx_labels], labels=FALSE, cex.axis=4, lty=1, tck=-0.01)
  #axis(side=2, at=seq(ymin,ymax,yseq),las=1,cex.axis=4.0)
  print(seq(ymin,ymax,yseq))
  axis(side=2, at=format(seq(ymin,ymax,yseq), scientific=TRUE, digits=2),las=1,cex.axis=4)
  box(lwd=2)
  abline(h=seq(ymin,ymax,yseq), lwd=1.0, col="grey")
